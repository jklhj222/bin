		...
		do {
			...
			if (draw_select)
			{
				...
				if (y_start >= preview.rows)
				{
					...
					rectangle(frame, selected_rect, Scalar(150, 200, 150));
                                        ...
                                        // jklhj draw cross diagonal line when draging the bbox
                                        line(frame, 
                                             Point2i(max(0, (int)min(x_start, x_end)), max(preview.rows, (int)min(y_start, y_end))), 
                                             Point2i(max(x_start, x_end), max(y_start, y_end)),
                                             Scalar(150, 200, 150));
                                        ...
                                        line(frame, 
                                             Point2i(max(0, (int)max(x_start, x_end)), max(preview.rows, (int)min(y_start, y_end))), 
            ...
            for(size_t k = 0; k < current_coord_vec.size(); ++k)
			{
				...
				rectangle(full_image_roi, i.abs_rect, color_rect, mark_line_width);
                                ...
                                // jklhj  plot cross diagonal lines
                                xmin = (int)i.abs_rect.x;
                                ymin = (int)i.abs_rect.y;
                                xmax = xmin + (int)i.abs_rect.width;
                                ymax = ymin + (int)i.abs_rect.height;
