...
darknet_path='/home/hugh/pkg/local/AlexeyAB_darkent_origin_v7/darknet'
gpu_idx='0,1'
...
then
    ...
    sed -i "$nfilter2""cfilters = $filter" train.cfg
    sed -i "$nfilter3""cfilters = $filter" train.cfg
elif [ $arch = 'yolov7-tiny' ]
then
    echo in yolov7-tiny
    cp yolov7-tiny.cfg train.cfg
    nfilter1=`echo $(grep -n yolo train.cfg | head -1 | awk -F ':' '{print $1}') - 4 | bc`
    nfilter2=`echo $(grep -n yolo train.cfg | head -2 | tail -1 | awk -F ':' '{print $1}') - 4 | bc`
    nfilter3=`echo $(grep -n yolo train.cfg | tail -1 | awk -F ':' '{print $1}') - 4 | bc`
    echo $nfilter1   $nfilter2   $nfilter3
    sed -i "$nfilter1""cfilters = $filter" train.cfg
    sed -i "$nfilter2""cfilters = $filter" train.cfg
    sed -i "$nfilter3""cfilters = $filter" train.cfg
...
"$darknet_path" detector train task.data train.cfg -map  -dont_show -gpus "$gpu_idx"
