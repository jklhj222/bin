...
print('df_dropna: ', df)
df.drop_duplicates(subset=check_col, keep='first', inplace=True)
print('df_dropdup: ', df)
...
noimg_idx_list = []
yesimg_idx_list = []
for idx, col in enumerate(check_df[check_col]):
...
if len(yesimg_idx_list) > 0:
    ...
    df_yesimg.to_csv('yesimg.csv', index=False, header=False)

if len(noimg_idx_list) > 0:
    df_noimg = df.drop(yesimg_idx_list, axis=0)
    df_noimg.to_csv('noimg.csv', index=False, header=False)
